...
(function () {
  "use strict";
  const ui = window.CWMicrophoneStream;
  if (ui.mountControls) return;
  ...
  function start(api, node, readScope, config) {
    ...
    let watcher;
    const streamer = ui.createStreamer({
      api: {
        ...
        applyAction(action, values) {
          ...
          return api.blockRequest("ui-action", { method: "POST", body: JSON.stringify({
            kind: "microphone_stream", node: { ...node, id: scope.nodeId },
   ...
   */
  ui.mountControls = function mountControls(root, api, context, config) {
